...
const { getDiscountRelationship } = require('../firestore/discount');
const { getLocationRelationship } = require('../firestore/location');
const {
...
const {  
  ...
  customerUpdate,
  customerDelete  
} = require('../graphQL/mutations/customer');
...
} = require('../graphQL/variables/customer');
const {
  collectionVariableMutationCreate,
  collectionVariableMutationDelete
} = require('../graphQL/variables/collection');
const {
  ...
  try{
    ...
    if(cloudbizProductsCount > firestoreProductsCount && cloudbizProductsCount > shopifyProductsCount){
        ...
        if(item.type === "product"){
          ...
          if(createProduct.productCreate.product.id != undefined){
            ...
            if(createVariantProduct.productVariantCreate.productVariant.id != undefined){
              await insertProductRelationship(createProduct.productCreate.id,createVariantProduct.productVariantCreate.productVariant.id,item);
            }
          }
        }
      });
      ...
      cloudbizProducts.forEach(async item => {
        if(item.type === "product"){
          ...
          if(createProduct.productCreate.id != undefined){
            var productVariant = await productVariantVariableMutationUpdate(createProduct.productCreate.id,item.code,null,taxable,item.title,totalPrice,null,item.image,inventory,categoryRelation.shopifyReference);
            var createVariantProduct = await graphQLClient(productVariantCreate,productVariant);
            if(createVariantProduct.productVariantCreate.productVariant.id != undefined){
              await insertProductRelationship(createProduct.productCreate.id,createVariantProduct.productVariantCreate.productVariant.id,item);
            }
      ...
      if(deleteProduct.length > 0){
        deleteProduct.forEach(async deleted => {
          var ProductVariable = await productVariableMutationDelete(deleted.data.shopifyReference);
          var ProductVariantVariable = await productVariantVariableMutationDelete(deleted.data.shopifyVariantReference);
          var queryProduct = await graphQLClient(productDeleteMutation,ProductVariable);
          var queryVariant = await graphQLClient(productVariantDelete,ProductVariantVariable);
          if((queryProduct && queryVariant) && queryProduct.productDelete != undefined && queryProduct.productDelete.deletedProductId != undefined){
            await deleteProductRelationship(deleted.data.shopifyReference,null,null);
            status = true;
          } 
        });
      }
